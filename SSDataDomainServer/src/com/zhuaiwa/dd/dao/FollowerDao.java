...
public class FollowerDao extends AbstractDao<Follower> {
    ...
    public List<FollowInfo> getFollower(String userid, FollowInfo cursor, int count, boolean reversed, BooleanHolder eol) {
        assert(count > 0);
        ...
        SuperSliceIterator<String, Composite, String, ByteBuffer> i = new SuperSliceIterator<String, Composite, String, ByteBuffer>(query, c, (Composite)null, reversed, 100);
        // 丢弃第一个，因为与cursor相同
        if (i.hasNext()) {
            HSuperColumn<Composite, String, ByteBuffer> superColumn = i.next();
            if (c == null || superColumn.getName().compareTo(c) != 0) {
                if (superColumn.getSize() > 0 && result.size() < count) {
                    result.add(buildFollowInfo(superColumn.getColumns()));
                }
            }
        }
        while (i.hasNext() && result.size() < count) {
            HSuperColumn<Composite, String, ByteBuffer> superColumn = i.next();
