...
public class ClusterMutex implements Watcher {
	...
	// Lazy on purpose. People who do not want mutex, should not need to have to
	// worry about ZK
	...
	// if we're disconnected from ZooKeeper server, how many times shall we try
	// the lock
	...
	private ClusterMutex() {
		String zooKeeperRoot = DDProperties.getZooKeeperRoot();
		...
		String zooKeeperPort = DDProperties.getZooKeeperPort();

		for (String zooKeeper : DDProperties.getZooKeeperAddresses()) {
			logger.warn(zooKeeper);
			hostString += (hostString.isEmpty()) ? "" : ",";
			if (zooKeeper.indexOf(":") != -1) {
				hostString += zooKeeper;
			} else {
		...
		try {
			...
			if (!root.isEmpty() && zk.exists(root, false) == null) {
				logger
						.info("Mutex root " + root
								+ " does not exists, creating");
				zk.create(root, new byte[0], Ids.OPEN_ACL_UNSAFE,
						CreateMode.PERSISTENT);
			}
		} catch (Exception e) {
			throw new RuntimeException("ClusterMutex initialization failed: "
					+ e.getMessage());
		}
	}
		...
		if ((now - lastConnect) < SessionTimeout) {
			if (logger.isTraceEnabled())
				logger
						.trace("Only "
								+ (now - lastConnect)
								+ "ms passed since last reconnect, not trying again yet");
			return;
		}
	...
	public void process(WatchedEvent event) {
		if (logger.isTraceEnabled())
			logger.trace("Got event " + event.getType() + ", keeper state "
	 ...
	 */
	public String lock(String lockName) throws KeeperException,
		...
		for (int i = 1; i <= OperationRetries; i++) {
			try {
				...
				// ZooKeeper handles lost connection automatically, but in order
				// to reset all
	 ...
	 */
	private String createLockZNode(String lockPath) throws KeeperException,
			InterruptedException {
		String lockZNode = null;

		try {
			lockZNode = zk.create(lockPath + "/" + LockPrefix, new byte[0],
					Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL_SEQUENTIAL);
		} catch (NoNodeException e) {
			logger.info(lockPath + " does not exist, creating");
			zk.create(lockPath, new byte[0], Ids.OPEN_ACL_UNSAFE,
					CreateMode.PERSISTENT);
			lockZNode = zk.create(lockPath + "/" + LockPrefix, new byte[0],
					Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL_SEQUENTIAL);
		}
	 ...
	 *            as in lock
	 */
	private String lockInternal(String lockName) throws KeeperException,
			InterruptedException {
		String lockZNode = null;
		...
		while (true) {
			...
			// before us, if
			// any. This must be inside loop, as children might get deleted out
			// of order because
			// of client disconnects. We cannot assume that the file that is in
			// front of us this
			// time, is there next time. It might have been deleted even though
			// earlier files
			// are still there.
			List<String> children = zk.getChildren(lockPath, false);
			if (children.isEmpty()) {
				logger.warn("No children in " + lockPath
						+ " although one was just created. Going to try again");
				lockZNode = createLockZNode(lockPath);
				continue;
			}
			...
			synchronized (mutex) {
				...
				if (isConnected() == false) {
					logger
							.info("ZooKeeper disconnected while waiting for lock");
					throw new KeeperException.ConnectionLossException();
				}
			}
		}
