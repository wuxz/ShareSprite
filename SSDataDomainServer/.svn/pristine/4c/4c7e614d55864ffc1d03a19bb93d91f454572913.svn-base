...
//	}
	...
	public static byte[] Object2ByteArray(Object o) {
		return Object2ByteArray(o, o.getClass());
	}
	
	static public byte[] Object2ByteArray(Object o, Class<?> type) {
	    return SerializerTypeInferer.getSerializer(type).toBytes(o);
	}
...
//	}
	...
	static public <Type> Type ByteArray2Object(ByteBuffer bytes, Class<Type> type) {
        if (type.isArray() && byte.class.isAssignableFrom(type.getComponentType())) {
            ...
            if (bytes.remaining() >= 2 && bytes.get(bytes.position()) == (byte) 0xac && bytes.get(bytes.position()+1) == (byte) 0xed) {
                ByteArrayInputStream bais = null;
                ObjectInputStream ois = null;
                Type retValue = null;
                try {
                    if (bytes.hasArray()) {
                        bais = new ByteArrayInputStream(bytes.array(), bytes.arrayOffset()+bytes.position(), bytes.remaining());
                    } else {
