...
public class HttpRequestHandler extends SimpleChannelUpstreamHandler
{
			...
			throws Exception
	{
		if (!readingChunks)
		{
			...
			responseContent.append("METHOD: " + request.getMethod().getName()
					+ "\r\n");

			if (request.containsHeader(HttpHeaders.Names.HOST))
			{
				responseContent.append("HOSTNAME: "
			...
			if (!request.getHeaderNames().isEmpty())
			{
				for (String name : request.getHeaderNames())
				{
					...
					{
						responseContent.append("HEADER: " + name + " = "
								+ value + "\r\n");
					}
				}
				responseContent.append("\r\n");
	...
	private void writeResponse(MessageEvent e)
	{
		...
		ChannelBuffer buf = ChannelBuffers.copiedBuffer(
				responseContent.toString(), "UTF-8");
