...
public class Indexer {
	...
	private static String DataDomainHost = PropertiesHelper.getValue(
			"cassandraDomainHost", "p0");

	private static int DataDomainPort = Integer.parseInt(PropertiesHelper
			.getValue("cassandraDomainPort", "29162"));

	private static Logger LOG = LoggerFactory.getLogger(Indexer.class);

	static {
		System.out.println("Indexer:: DataDomain:" + DataDomainHost + "/"
				+ DataDomainPort);
	}
	...
	private static Profile getProfile(String userid, Cassandra.Client cassandra)
			throws Exception {
	...
	private static void updateMessage(String msgId) throws Exception{
		...
		cassandra = new Cassandra.Client(new TBinaryProtocol(new TFramedTransport(socket)));
		cassandra.set_keyspace("SSDataDomain");
		ReadCommand command = new ReadCommand(cassandra).Object(Message.class).Select().Where(msgId);
...
//				Indexer.getInstance().addMessage(mb.build());
	...
	private static void updateUser(String userId) throws Exception{
		...
		cassandra = new Cassandra.Client(new TBinaryProtocol(new TFramedTransport(socket)));
		cassandra.set_keyspace("SSDataDomain");
		ReadCommand command = new ReadCommand(cassandra).Object(Account.class).Select().Where(userId);
		...
		Profile profile = getProfile(account.getUserid(), cassandra);
		if (profile != null) {
