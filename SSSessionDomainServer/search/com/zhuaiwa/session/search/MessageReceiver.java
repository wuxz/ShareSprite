...
public class MessageReceiver
{
	public static class Indexer implements Runnable
	{
		...
		public Indexer(Operate operate, SolrObj obj)
		{
			...
			this.obj = obj;
			tip = operate.name() + " "
					+ SolrObjType.values()[obj.getObjType()].name() + ":"
					+ obj.getId();
		...
		private void delete()
		{
			try
			{
				...
				if (obj.getObjType() == SolrObjType.USER.ordinal())
				{
					...
					query += "(userId:" + obj.getId() + ")AND((objType:"
							+ SolrObj.SolrObjType.EDUCATION.ordinal()
	...
	static
	{
		System.out.println("MessageReceiver:: brokerURL:" + brokerURL
				+ ", queueName:" + queueName + ", solrURL:" + solrURL);
		try
		{
	...
	public void receive()
	{
		...
		final ThreadPoolExecutor threadPoolExecutor = new ThreadPoolExecutor(5,
				10, 5, TimeUnit.MINUTES, new LinkedBlockingQueue<Runnable>(
						10000));
		...
		scheduledThreadPoolExecutor.scheduleAtFixedRate(new Runnable()
		{
			...
			public void run()
			{
				...
				LOG.info("threadPoolExecutor Active:"
						+ threadPoolExecutor.getActiveCount() + ", Completed:"
						+ threadPoolExecutor.getCompletedTaskCount()
						+ ", Task:" + threadPoolExecutor.getTaskCount()
						+ ", Queue size:"
			...
			try
			{
				...
				while (true)
				{
					...
					if (null != message)
					{
						...
						LOG.debug("receive a message.msg type:"
								+ message.getJMSType()
								+ ", msg class:"
								...
								if (message instanceof ObjectMessage)
								{
									...
									if (om.getObject() instanceof SolrObj)
									{
										...
										while (!succeeded)
										{
											...
											{
												LOG.warn("can not find a free thread to handle message.");
												Thread.sleep(1000);
												succeeded = false;
											}
										}
